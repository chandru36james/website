...
const Contact: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (prefilledMessage) {
            setFormState(prev => ({ ...prev, message: prefilledMessage }));
        }
    }, [location.search]);
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!accessKey) {
                ...
                const mailtoUrl = `mailto:info@vgotyou.com?subject=Project Inquiry from ${encodeURIComponent(formState.name)}&body=Name: ${encodeURIComponent(formState.name)}%0D%0AEmail: ${encodeURIComponent(formState.email)}%0D%0AMobile: ${encodeURIComponent(formState.mobile)}%0D%0AMessage: ${encodeURIComponent(formState.message)}`;
                const whatsappUrl = `https://wa.me/917871120415?text=${encodeURIComponent(`Hi VGot You, I filled out the contact form. Name: ${formState.name}, Email: ${formState.email}, Mobile: ${formState.mobile}. Message: ${formState.message}`)}`;

                Swal.fire({
                    title: "Access Key Required for API Sending",
                    html: `
                        <div class="text-left text-zinc-300 text-sm space-y-3">
                            <p>To deliver submissions directly via API to your email, please set the <code class="bg-black px-1.5 py-0.5 rounded text-red-400 font-mono text-xs">VITE_WEB3FORMS_ACCESS_KEY</code> environment variable in your AI Studio settings.</p>
                            <p class="text-xs text-zinc-500">Get a free key instantly from <a href="https://web3forms.com" target="_blank" rel="noopener noreferrer" class="text-red-400 underline">web3forms.com</a>.</p>
                            <p class="font-bold pt-2 text-white">Meanwhile, you can send your message immediately using these fallback links:</p>
                        </div>
                    `,
                    icon: "warning",
                    showCancelButton: true,
                    showDenyButton: true,
                    confirmButtonText: "📧 Send via Email Client",
    ...
    const faqs = [
        ...
        {
            q: "How are your web design projects structured?",
            a: "Every project is fully tailored based on your business objectives and design needs. After an initial discussion of requirements, we provide a detailed proposal outlining the scope, deliverables, and timeline. Custom design phases include draft reviews and revisions."
        },
        {
    ...
    const contactInfo = [
        { label: 'Email', value: 'info@vgotyou.com', href: 'mailto:info@vgotyou.com?subject=Project%20Inquiry%20with%20VGot%20You&body=Hi%20VGot%20You%2C%0D%0A%0D%0AI%20am%20interested%20in%20discussing%20a%20new%20web%20design%20and%20development%20project%20with%20you.%20Please%2520get%2520in%2520touch%21', icon: <EmailIcon className="w-5 h-5 md:w-6 md:h-6" /> },
  ...
  {/* FIXED: vgotyou.png → vgotyou.png, consistent across all pages */}
                ...
                <div className="grid lg:grid-cols-12 gap-8 md:gap-16 items-start">
                    ...
                    <div className="lg:col-span-4 space-y-6 md:space-y-12">
                        <FadeInSection>
                            <h2 className="text-xl md:text-3xl font-bold mb-5 md:mb-8 flex items-center gap-3">
                                <span className="w-4 h-1 bg-red-600 rounded-[10px]"></span>
                                Reach Out
                            </h2>
                            <div className="space-y-4 md:space-y-8">
                                {contactInfo.map(info => (
                                    <div key={info.label} className="group p-4 md:p-6 rounded-[10px] bg-zinc-900 border border-zinc-800 transition-all hover:border-red-600/30">
                                        <div className="flex items-center gap-4">
                                            <div className="w-9 h-9 md:w-12 md:h-12 rounded-[10px] bg-red-600/10 flex items-center justify-center text-red-600 group-hover:scale-110 transition-transform">
                                                {info.icon}
                                            </div>
                                            <div>
                        ...
                        <FadeInSection delay={0.2}>
                            <div className="p-5 md:p-8 rounded-[10px] bg-gradient-to-br from-red-600 to-red-900 text-white shadow-xl">
                    ...
                    <div className="lg:col-span-8">
                        <FadeInSection delay={0.1}>
                            <div className="bg-zinc-900 p-5 md:p-12 rounded-[10px] border border-zinc-800 shadow-2xl">
                                <form className="grid md:grid-cols-2 gap-4 md:gap-8" onSubmit={handleFormSubmit}>
                                    <div className="space-y-1.5 md:space-y-2">
                                        ...
                                        <input 
                                            ...
                                            className="w-full bg-black border border-zinc-800 rounded-[10px] p-3 md:p-4 text-xs md:text-base text-white focus:outline-none focus:ring-1 focus:ring-red-600/50 focus:border-red-600 transition-all placeholder:text-zinc-700" 
                                        />
                                    </div>
                                    <div className="space-y-1.5 md:space-y-2">
                                    ...
                                    <div className="md:col-span-2 space-y-1.5 md:space-y-2">
                                        ...
                                        <textarea 
                                            ...
                                            className="w-full bg-black border border-zinc-800 rounded-[10px] p-3 md:p-4 text-xs md:text-base text-white focus:outline-none focus:ring-1 focus:ring-red-600/50 focus:border-red-600 transition-all placeholder:text-zinc-700 resize-none"
                                        ></textarea>
                                    </div>
                                    <div className="md:col-span-2 pt-2 md:pt-4">
                                        <button
                                            ...
                                            className="w-[180px] sm:w-[195px] inline-flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white font-bold px-4 py-2.5 rounded-[10px] transition-all duration-300 text-[11px] sm:text-xs uppercase tracking-wider shadow-lg shadow-red-600/20 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95"
                                        >
