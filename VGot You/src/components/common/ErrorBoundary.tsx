      ...
      return (
        <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-6 text-center">
          ...
          <h1 className="text-2xl font-black text-white uppercase tracking-tighter mb-4">
            {isFirestoreError ? 'Database Access Error' : 'System Diagnostic: Error Detected'}
          </h1>
          <p className="text-zinc-500 max-w-md mb-8 text-sm leading-relaxed">
            {isFirestoreError 
              ? "We're having trouble accessing the database. This might be due to permission issues or a connection problem."
          ...
          <div className="flex flex-col gap-4">
            ...
            <button
              ...
              className="px-8 py-3 bg-zinc-800 text-white font-bold text-[10px] uppercase tracking-[0.3em] rounded-full hover:bg-zinc-700 transition-all"
            >
              Return Home
            </button>
          </div>
            ...
            <pre className="text-zinc-400 font-mono text-[10px] whitespace-pre-wrap">
              ...
              {firestoreDetails && (
                <div className="mt-2 pt-2 border-t border-zinc-800">
                  <p className="text-blue-400">Auth Status: {firestoreDetails.authInfo.userId ? 'Authenticated' : 'Anonymous'}</p>
                  <p className="text-blue-400">User ID: {firestoreDetails.authInfo.userId || 'N/A'}</p>
                </div>
              )}
            </pre>
          </div>
