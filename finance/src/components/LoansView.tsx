...
export const LoansView: React.FC = () => {
  const { 
    ...
    requestSettlement, settlements, approveSettlement, rejectSettlement,
    selectedCustomerIdForNewLoan, setSelectedCustomerIdForNewLoan
  } = useVFMS();
  ...
  React.useEffect(() => {
    if (selectedCustomerIdForNewLoan) {
      setCustomerId(selectedCustomerIdForNewLoan);
      setActiveSubTab('create');
      setSelectedCustomerIdForNewLoan(null);
    }
  ...
  const getOverdueDate = (loanId: string) => {
    const loanScheds = schedules[loanId] || [];
    const overdueScheds = loanScheds.filter(s => s.status === 'Overdue');
    if (overdueScheds.length === 0) return null;

    const sorted = [...overdueScheds].sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime());
    const graceDays = financeRules[0]?.gracePeriodDays || 0;
    const dueDate = new Date(sorted[0].dueDate);
  ...
  const handleCreateLoan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Loan Created', 'The loan application was submitted successfully.');
      ...
      toast.error('Application Failed', err.message || 'Could not submit loan application.');
    }
  };
  ...
  const handleRepayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Payment Received', 'The repayment was successfully registered and allocated.');
      ...
      toast.error('Payment Failed', err.message || 'Could not register repayment.');
    }
  };
  ...
  const handleWaiveSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Installment Waived', 'The waiver has been authorized.');
      ...
      console.error('Waive installment error:', err);
      toast.error('Waiver Failed', err.message || 'Could not authorize waiver.');
    }
  };
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  {filteredLoans.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-12 text-center text-slate-400 font-bold">No active loans.</td>
                    </tr>
                  ) : (
            ...
            {selectedLoan ? (
              <div className="bg-white rounded-[10px] border border-slate-200 shadow-sm p-4 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-y-3.5 gap-x-2 text-xs">
                  ...
                  {selectedLoan.status === 'Overdue' && getOverdueDate(selectedLoan.id) && (
                    <div className="col-span-2 p-2 bg-red-50 border border-red-200 rounded-lg flex items-center justify-between text-red-800 font-semibold mt-1">
                      <span className="text-[9px] font-bold uppercase">Arrears Overdue Since:</span>
                      <span className="font-mono text-[10px] text-red-700">{getOverdueDate(selectedLoan.id)}</span>
                    </div>
                  )}
                </div>
                  ...
                  <div className="pt-2 border-t border-slate-100 space-y-2">
                        ...
                        <button
                          onClick={async () => {
                            try {
                              ...
                              toast.success('Loan Approved', `Application for ${selectedLoan.customerName} has been approved.`);
                            } catch (err: any) {
                              console.error('Approve loan error:', err);
                              toast.error('Approve Failed', err.message || 'Could not approve loan.');
                            }
                          }}
                        ...
                        <button
                          onClick={async () => {
                            try {
                              ...
                              toast.success('Loan Rejected', `Application for ${selectedLoan.customerName} has been rejected.`);
                            } catch (err: any) {
                              console.error('Reject loan error:', err);
                              toast.error('Reject Failed', err.message || 'Could not reject loan.');
                            }
                          }}
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            toast.success('Loan Disbursed', `Funds disbursed and ledger schedule generated for ${selectedLoan.customerName}.`);
                          } catch (err: any) {
                            console.error('Disburse loan error:', err);
                            toast.error('Disbursement Failed', err.message || 'Could not disburse loan.');
                          }
                        }}
                    ...
                    {selectedLoan.status === 'Overdue' && userRole === 'Super Admin' && (
                      <button
                        onClick={async () => {
                          try {
                            await writeOffLoan(selectedLoan.id, 'Bad Debt Write-off');
                            toast.success('Loan Written Off', `Bad debt write-off logged for ${selectedLoan.customerName}.`);
                          } catch (err: any) {
                            console.error('Write-off loan error:', err);
                            toast.error('Write-off Failed', err.message || 'Could not underwrite write-off.');
                          }
                        }}
                    ...
                    {['Active', 'Overdue'].includes(selectedLoan.status) && (
                      <div className="border-t border-slate-100 pt-2.5 space-y-2">
                        <button
                          type="button"
                          onClick={() => setIsSettlementFormOpen(!isSettlementFormOpen)}
                          className="w-full py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-[11px] font-bold rounded-[10px] flex items-center justify-center gap-1 cursor-pointer transition-all border border-indigo-150"
                        >
                          <Scale className="w-3.5 h-3.5" /> 
                          {isSettlementFormOpen ? "Close Settlement Drawer" : "Request Early Settlement"}
                        </button>

                        {isSettlementFormOpen && (
                          <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-2.5 animate-fade-in text-[11px]">
                            <div className="space-y-1">
                              <label className="font-bold text-slate-700">Settlement Discount (Waiver Amount) (₹)</label>
                              <input
                                type="number"
                            ...
                            <div className="space-y-1">
                              <label className="font-bold text-slate-700">Settlement Reasons / Notes</label>
                              <input
                                type="text"
                                value={settlementNotes}
                                onChange={e => setSettlementNotes(e.target.value)}
                                placeholder="e.g. Clearing outstanding amount early"
                                className="w-full px-2.5 py-1.5 border border-slate-200 rounded-lg focus:outline-none"
                              />
                            </div>
                            <button
                              type="button"
                              onClick={async () => {
                                const disc = parseFloat(settlementDiscount);
                                if (isNaN(disc) || disc < 0) {
                                  alert("Please enter a valid discount amount.");
                                  return;
                                }
                                try {
                                  await requestSettlement(selectedLoan.id, disc, settlementNotes);
                                  toast.success("Settlement Requested", "Early closure request has been submitted to the manager.");
                                  setIsSettlementFormOpen(false);
                                  setSettlementDiscount('0');
                                  setSettlementNotes('');
                                } catch (err: any) {
                                  toast.error("Request Failed", err.message || "Failed to submit settlement request.");
                                }
                              }}
                              className="w-full py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg cursor-pointer transition-colors"
                            >
                              Submit Early Settlement Claim
                            </button>
                          </div>
                        )}
                      </div>
                    ...
                    {/* Manage Settlement Approvals (If pending settlement exists) */}
                    {settlements.filter(s => s.loanId === selectedLoan.id && s.status === 'Pending Approval').map(s => (
                      <div key={s.id} className="p-3 bg-amber-50 border border-amber-200 rounded-xl space-y-2 text-[11px]">
                        <span className="font-bold text-amber-800">Pending Settlement Request</span>
                        <div className="text-[10px] text-slate-600 font-mono space-y-0.5">
                          <p>Discount Claim: ₹{s.discountAmount}</p>
                          <p>Net Payout Required: ₹{s.settlementAmount}</p>
                          {s.notes && <p>Reason: "{s.notes}"</p>}
                        </div>
                            ...
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  toast.success("Settlement Approved", "The loan contract has been closed successfully.");
                                } catch (err: any) {
                                  toast.error("Operation Failed", err.message);
                                }
                              }}
                              className="flex-1 py-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg cursor-pointer text-[10px] text-center"
                            >
                              Approve
                            </button>
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  await rejectSettlement(s.id);
                                  toast.success("Settlement Rejected", "Early closure request has been rejected.");
                                } catch (err: any) {
                                  toast.error("Operation Failed", err.message);
                                }
                              }}
                              className="flex-1 py-1 bg-red-650 hover:bg-red-750 text-white font-bold rounded-lg cursor-pointer text-[10px] text-center"
                            >
                              Reject
                            </button>
                          </div>
