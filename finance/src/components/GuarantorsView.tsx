...
export const GuarantorsView: React.FC = () => {
  ...
  const getAssociatedCustomers = (guar: Guarantor) => {
    const linkedLoans = loans.filter(l => guar.loanIds.includes(l.id));
    const customerMap = new Map<string, string>(); // customerId -> customerName
    linkedLoans.forEach(l => {
      customerMap.set(l.customerId, l.customerName);
    });
  ...
  const validateField = (fieldName: string, value: string) => {
    let errorMsg = '';
    if (fieldName === 'name') {
      if (!value.trim()) errorMsg = 'Guarantor name is required.';
    } else if (fieldName === 'mobile') {
      if (!value.trim()) {
        errorMsg = 'Mobile phone number is required.';
      } else if (!/^\d{10}$/.test(value.trim())) {
        errorMsg = 'Mobile phone number must be exactly 10 digits.';
      }
    } else if (fieldName === 'nationalId') {
      if (!value.trim()) {
        errorMsg = 'Aadhaar / National ID is required.';
      } else if (!/^\d{12}$/.test(value.trim())) {
        errorMsg = 'Aadhaar / National ID must be exactly 12 digits.';
      }
  ...
  const validateForm = () => {
    ...
    const nationalIdValid = validateField('nationalId', nationalId);
    ...
    if (!signatureFinalData) {
      signatureError = 'Guarantor signature authorization is required.';
    }
    setErrors(prev => ({ ...prev, signature: signatureError }));

    let loanError = '';
    if (selectedLoanIds.length === 0) {
      loanError = 'At least one associated loan contract must be linked.';
    }
    setErrors(prev => ({ ...prev, loans: loanError }));

    return nameValid && mobileValid && nationalIdValid && addressValid && !signatureError && !loanError;
  };
  ...
  const handleToggleLoanSelection = (loanId: string) => {
    ...
    if (selectedLoanIds.includes(loanId)) {
      nextLoans = selectedLoanIds.filter(id => id !== loanId);
    } else {
      nextLoans = [...selectedLoanIds, loanId];
    }
    setSelectedLoanIds(nextLoans);
    if (errors.loans && nextLoans.length > 0) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validateForm()) {
      toast.error('Validation Warning', 'Please resolve the highlighted validation errors before saving.');
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        toast.success('Guarantor Updated', `${name}'s guarantor folder has been updated.`);
      } else {
        await addGuarantor({
          name,
          nationalId,
        ...
        toast.success('Guarantor Registered', `Successfully onboarded ${name} as a co-signer.`);
      }

      setIsModalOpen(false);
      setSelectedGuarantor(null);
    } catch (err: any) {
      console.error('Guarantor submit error:', err);
      toast.error('Operation Failed', err.message || 'Could not save guarantor records.');
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to release this guarantor from active co-signer obligations?')) {
      try {
        ...
        toast.success('Guarantor Released', 'The co-signer obligation has been revoked.');
        setSelectedGuarantor(null);
      } catch (err: any) {
        console.error('Guarantor release error:', err);
        toast.error('Release Failed', err.message || 'Could not release guarantor from obligation.');
      }
    }
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              />
              {errors.name && <p className="text-[10px] text-red-600 font-bold mt-0.5">{errors.name}</p>}
            </div>
            ...
            <div className="space-y-1">
              ...
              <input 
                ...
                onChange={(e) => {
                  ...
                  if (errors.nationalId) validateField('nationalId', e.target.value);
                }}
                onBlur={(e) => validateField('nationalId', e.target.value)}
                placeholder="Enter National Identity code"
                className={`form-input ${errors.nationalId ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : ''}`}
              />
              {errors.nationalId && <p className="text-[10px] text-red-600 font-bold mt-0.5">{errors.nationalId}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input 
                ...
                className={`form-input ${errors.mobile ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : ''}`}
              />
              {errors.mobile && <p className="text-[10px] text-red-600 font-bold mt-0.5">{errors.mobile}</p>}
            </div>
          ...
          <div className="space-y-1">
            ...
            <textarea 
              ...
              rows={2}
              className={`form-input ${errors.address ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : ''}`}
            />
            {errors.address && <p className="text-[10px] text-red-600 font-bold mt-0.5">{errors.address}</p>}
          </div>

          {/* Linked Loans selection lists */}
          <div className="space-y-1.5">
            <label className="form-label">Assign / Co-Sign Portfolios *</label>
            <div className={`border rounded-[10px] p-2 max-h-32 overflow-y-auto space-y-1.5 bg-slate-50/50 ${
              errors.loans ? 'border-red-500 bg-red-50/5' : 'border-slate-200'
            }`}>
              {loans.length === 0 ? (
            ...
            {errors.loans && <p className="text-[10px] text-red-600 font-bold mt-0.5">{errors.loans}</p>}
          </div>

          {/* Secure Signature execution panel */}
          <div className={`border rounded-[10px] overflow-hidden bg-slate-50/50 ${
            errors.signature ? 'border-red-500 bg-red-50/5' : 'border-slate-200'
          }`}>
                ...
                <div className="space-y-2">
                  <input 
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.signature) {
                        setErrors(prev => ({ ...prev, signature: e.target.value ? '' : 'Guarantor signature authorization is required.' }));
                      }
                    }}
          ...
          {errors.signature && <p className="text-[10px] text-red-600 font-bold mt-0.5">{errors.signature}</p>}
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-white text-slate-400 font-bold text-[10px] uppercase border-b border-slate-100">
                  ...
                  <th className="px-4 py-2.5">Co-Signing For</th>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {filteredGuarantors.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-12 text-center text-slate-400 font-bold">No records found.</td>
                  </tr>
                ) : (
                  filteredGuarantors.map(guar => {
                    const assoc = getAssociatedCustomers(guar);
                    return (
                      <tr 
                        key={guar.id}
                      ...
                      >
                        <td className="px-4 py-2.5">
                          <span className="font-bold text-slate-900 block">{guar.name}</span>
                          <span className="text-[9px] text-slate-400 font-mono">ID: {guar.nationalId}</span>
                        </td>
                        <td className="px-4 py-2.5">
                          {assoc.length > 0 ? (
                            assoc.map(c => (
                              <div key={c.id} className="text-[10px] leading-tight">
                                ...
                                <span className="text-[8px] text-slate-400 font-mono">({c.id})</span>
                              </div>
                            ))
                          ) : (
                            <span className="text-slate-400 italic text-[10px]">No active link</span>
                          )}
                        </td>
            ...
            <div className="bg-white rounded-[10px] border border-slate-200 shadow-sm p-4 space-y-4">
              ...
              <div className="flex justify-between items-start border-b border-slate-150 pb-3">
                <div>
                  ...
                  <div className="mt-1 flex flex-col gap-0.5 text-[9px] font-semibold text-slate-500">
                    {getAssociatedCustomers(selectedGuarantor).length > 0 ? (
                      getAssociatedCustomers(selectedGuarantor).map(c => (
                        <span key={c.id} className="inline-flex items-center gap-1">
                          Co-signing for: <strong className="text-red-600">{c.name}</strong> <span className="text-[8px] font-mono text-slate-400">({c.id})</span>
                        </span>
                      ))
                    ) : (
                      <span className="text-slate-400 italic">No active customer link</span>
                    )}
                  </div>
                </div>
